...
export default function PurchaseDownPaymentsPage() {
  const crud = useCrudTable(API_ENDPOINTS.PURCHASE.DOWN_PAYMENTS, () => '');
  ...
  useEffect(() => {
    Promise.all([
      get(API_ENDPOINTS.MASTER.SUPPLIERS, { limit: 100 }),
      get(API_ENDPOINTS.MASTER.CASH_BANK),
    ]).then(([s, a]) => { setSuppliers(s.data); setAccounts(a.data); });
  }, []);
  ...
  const handlePost = async (row) => {
    await post(API_ENDPOINTS.PURCHASE.DOWN_PAYMENT_POST(row.id));
    toast.success('DP diposting'); crud.refresh();
  };

  return (
    <div>
      <PageHeader title="Uang Muka Pembelian" onAdd={() => { setForm({ dp_date: new Date().toISOString().split('T')[0] }); setModal(true); }} search={crud.search} onSearchChange={crud.setSearch} />
      <DataTable columns={columns} data={crud.data} loading={crud.loading} actions={(row) => row.status === 'draft' && <Button size="sm" onClick={() => handlePost(row)}>Post</Button>} />
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Uang Muka Pembelian">
        <div className="space-y-4">
          <Select label="Pemasok" value={form.supplier_id || ''} onChange={(e) => setForm({ ...form, supplier_id: e.target.value })}
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Button onClick={async () => { await post(API_ENDPOINTS.PURCHASE.DOWN_PAYMENTS, form); toast.success('DP dibuat'); setModal(false); crud.refresh(); }}>Simpan</Button>
        </div>
      </Modal>
    </div>
  );
